...
macro_rules! lisp {
    ...
    // Function call with arguments - this handles nested expressions
    (($func:ident $($args:tt)*)) => {
        $func($(lisp!($args)),*)
    };
...
fn main() {
    ...
    let result = lisp!((let x 3 (let y 4 (+ (* x x) (* y y)))));
    println!("(let x 3 (let y 4 (+ (* x x) (* y y)))) = {}", result);
    ...
    println!("\nComplex Example:");
    let result = lisp!((let a 3 (let b 4 (let c 5
        (+ (* a a) (* b b) (* c c))
    ))));
...
mod tests {
    ...
    fn test_let() {
        ...
        assert_eq!(lisp!((let x 3 (let y 4 (+ x y)))), 7);
    }
    ...
    fn test_function_calls() {
        ...
        assert_eq!(lisp!((add 1 2)), 3);
        assert_eq!(lisp!((add (+ 1 2) (+ 3 4))), 10);
    }
